...
use chrono::{Date, DateTime, TimeZone, NaiveDateTime, Utc};
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let max_stmt = format!("SELECT max(temperature) FROM {}.{} WHERE device={} AND ts >= ? AND ts <= ?", ks, table, uuid_search);
    let min_stmt = format!("SELECT min(temperature) FROM {}.{} WHERE device={} AND ts >= ? AND ts <= ?", ks, table, uuid_search);
    let avg_stmt = format!("SELECT avg(temperature) FROM {}.{} WHERE device={} AND ts >= ? AND ts <= ?", ks, table, uuid_search);

    let mut max_ps: PreparedStatement = session
                                .prepare(max_stmt).await?;
    let mut min_ps: PreparedStatement = session
                                .prepare(min_stmt).await?;
    let mut avg_ps: PreparedStatement = session
                                .prepare(avg_stmt).await?;
    ...
    println!();
    println!("\n\n{}", hdr);
    println!("Start querying {} from {} to {}\n", uuid_search, start_date, end_date);

    // Query data
    if let Some(rows) = session.execute(&max_ps, (Timestamp(to_start), Timestamp(to_end))).await?.rows {
    ...
    if let Some(rows) = session.execute(&min_ps, (Timestamp(to_start), Timestamp(to_end))).await?.rows {
       for row in rows.into_typed::<(f32,)>() {
              let (min,): (f32,) = row?;
    ...
    if let Some(rows) = session.execute(&avg_ps, (Timestamp(to_start), Timestamp(to_end))).await?.rows {
       for row in rows.into_typed::<(f32,)>() {
              let (avg,): (f32,) = row?;
